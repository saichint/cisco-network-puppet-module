# Manages a Cisco PlbDeviceGroupNode.
#
# October 2016
#
...
Puppet::Type.newtype(:cisco_plb_device_group_node) do
  @doc = "Manages a Cisco PlbDeviceGroupNode.

  cisco_plb_device_group_node {\"<node>\":
    ..attributes..
  }

  <plbdg> is the name of the plb device-group, and <node> is the name of the node instance.

  Example:
    cisco_plb_device_group_node {\"mygroup 1.1.1.1\":
     ensure                       => present,
     hot_standby                  => false,
     node_type                    => \"ip\",
     probe_control                => false,
  ...
  newparam(:plbdg, namevar: :true) do
    desc 'Name of the plb device group.
          Valid values are string.'

    validate do |value|
      fail('Plb device group name must be a string') unless value.is_a? String
    end
  ...
  newproperty(:plb_device_group) do
    desc 'Plb device group'

    validate do |grp|
      fail("plb_device_group property - #{grp} should be a string") unless
        grp.kind_of? String
    end
  end # property group
