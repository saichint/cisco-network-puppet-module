...
Puppet::Type.newtype(:cisco_plb_service) do
  @doc = "Manages a Cisco Plb Service.

  Any resource dependency should be run before the interface resource.

  cisco_plb_service {\"<name>\":
    ..attributes..
  }
  ...
  newparam(:service_name, namevar: :true) do
    desc "Name of the plb service. Valid value is a
  ...
  newproperty(:access_list) do
    desc "PLB access-list name. Valid values are string, keyword
         'default'. "
  ...
  newproperty(:device_group) do
    desc "PLB device-group name. Valid values are string, keyword
         'default'. "
  ...
  newproperty(:exclude_access_list) do
    desc "PLB exclude-access-list name. Valid values are string, keyword
         'default'. "
  ...
  newproperty(:fail_action) do
    desc "Failaction for PLB enables traffic on failed nodes to be
  ...
  newproperty(:ingress_interface, array_matching: :all) do
    ...
    munge do |value|
      ...
      begin
        value.each do |intf, _next_hop|
          intf.downcase! if intf[/[A-Z]/]
          intf.delete!(' ') if intf.include?(' ')
        end
